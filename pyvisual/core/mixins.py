	...
	def add_points(
		...
		**kwargs
		...
		.. pyvista-plot::
		    ...
		    >>> plotter.show()
		"""
		return self._add_stack_set(r, t, p, data, axis, dataid, slice_type="points", **kwargs)
	...
	def add_splines(
		...
		**kwargs
		...
		.. pyvista-plot::
		    ...
		    >>> plotter.show()
		"""
		return self._add_stack_set(r, t, p, data, axis, dataid, slice_type="splines", **kwargs)
...
class GridMeshMixin:
	...
	def add_1d_slice(
		...
		**kwargs
		...
		return self._add_grid_set(r, t, p, data, axis, dataid, slice_type="splines", **kwargs)
	...
	def add_2d_slice(
		...
		**kwargs
		...
		try:
			...
			msg = f"Could not infer slice axis from input shapes: {mesh_shape}"
			raise ValueError(msg) from e
		return self._add_grid_set(r, t, p, data, axis, dataid, slice_type="slices", **kwargs)
	...
	def add_3d_slice(
		...
		**kwargs
		...
		.. pyvista-plot::
		    ...
		    >>> plotter.show()
		"""
		return self._add_grid_set(r, t, p, data, axis, dataid, slice_type, **kwargs)
...
class ObserverMixin:
	...
	def add_fixed_solar_north_observer(self, p_angle: float = 0) -> None:
		"""
	...
	def add_camera_update(self, *, include: set[str] | str | None = None, **kwargs) -> None:
		...
		**kwargs
		...
		def _update_text(caller, event):		# noqa: ARG001
			values = [
				...
				else f"{opt}: {_format_tuple(getattr(self.camera, opt.removeprefix('camera_')))}"
				for opt in options
			]
			msg = "\n".join(values)
	...
	def observer_los_view(self, args) -> None:
		...
		fp = thompson_sphere(
			...
			latitude,
			radius,
			self.observer_orientation.p_angle,
		)
...
class GeometryMixin:
	...
	def add_shell(
		...
		**kwargs
		...
		shell_inner = pv.Sphere(
			radius=inner_radius, center=center, theta_resolution=180, phi_resolution=360
		)
		shell_outer = pv.Sphere(
			radius=outer_radius, center=center, theta_resolution=180, phi_resolution=360
		)
	...
	def add_disc(
		...
		**kwargs
		...
		disc = pv.Disc(
			center=center, inner=inner_radius, outer=outer_radius, normal=normal, c_res=360
		)
		return self.add_mesh(disc, **kwargs)

	def add_thompson_sphere(
		self, *pos, theta_resolution: int = 180, phi_resolution: int = 360, **kwargs
	) -> pv.Actor:
		r"""
		...
		**kwargs
		...
		tsphere = pv.Sphere(
			radius=np.linalg.norm(observer_position) / 2,
	...
	def add_longitudinal_lines(self, lon_deg: int = 30, radius: float = 1.01, **kwargs) -> pv.Actor:
		r"""
	...
	def add_latitudinal_lines(self, lat_deg: int = 15, radius: float = 1.01, **kwargs) -> pv.Actor:
	...
	def add_longlat_lines(
		self, lat_deg: int = 15, lon_deg: int = 30, radius: float = 1.01, **kwargs
	) -> pv.Actor:
