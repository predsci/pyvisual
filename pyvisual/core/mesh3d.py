...
class _BaseFrameMesh(ABC):
	...
	def __init__(
		...
		**kwargs
		...
		super().__init__()
		if args:
			self._dispatch_input(*args, data=data, dataid=dataid, iformat=iformat, **kwargs)
	...
	@singledispatchmethod
	def _dispatch_input(self, uinput, *args, **kwargs):
	...
	def _(self, uinput, *args, data, dataid, iformat, **kwargs):
	...
	def _(self, uinput, *args, data, iformat, **kwargs):
	...
	**kwargs
	...
	def _parse_iformat(self, iformat: str | None) -> tuple[str, str]:
		...
		if not iformat:
			...
			try:
				...
				msg = f"Unrecognized format string. Expected one of: {set(FRAME_ALIASES.keys())}. "
				raise ValueError(msg) from e
	...
	def _dispatch_pyvista(self, uinput, iformat, deep):
		if isinstance(uinput, (pv.StructuredGrid, pv.RectilinearGrid, pv.ImageData)):
			...
			msg = (
				f"Cannot construct CartesianMesh from {type(uinput).__name__}; "
	...
	**kwargs
	...
	def _dispatch_pyvista(self, uinput, iformat, deep):
		if isinstance(uinput, (pv.RectilinearGrid, pv.ImageData)):
			...
			msg = (
				f"Cannot construct SphericalMesh from {type(uinput).__name__}; "
	...
	def __getitem__(self, key):
		...
		scales = np.atleast_1d(
			*(scale[index].copy() for scale, index in zip(self.scales, key, strict=True))
		)
