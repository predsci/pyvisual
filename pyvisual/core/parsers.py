...
def _align_shape(values, mesh_shape):
    ...
    for size in val_shape:
        matches = [i for i, s in enumerate(mesh_shape) if s == size and i not in
                   mesh_axes]
        if not matches:
...
def generate_transforms(mframe, pframe):
    ...
    if ftrans := _FRAME_TRANSFORM_MAPPING.get(frame_key, None):
...
def parse_stack_mesh(r: np.ndarray,
    ...
    if r.shape == t.shape == p.shape:
        return r, t, p
    raise ValueError(f"All scales must be ND (with identical shapes), "
...
def parse_grid_mesh(r: np.ndarray,
    ...
    if 1 == r.ndim == t.ndim == p.ndim:
        ...
        return r, t, p
    msg = (f"All scales must be 1D (with arbitrary lengths), or 3D (with identical shapes), "
           f"got r: shape={r.shape}, t: shape={t.shape}, p: shape={p.shape}.")
    raise ValueError(msg)
