...
def spherical_to_cartesian(
	r: ArrayLike, t: ArrayLike, p: ArrayLike
) -> tuple[ArrayLike, ArrayLike, ArrayLike]:
...
def cartesian_to_spherical_vec(
	vr: ArrayLike, vt: ArrayLike, vp: ArrayLike, t: ArrayLike, p: ArrayLike
) -> tuple[ArrayLike, ArrayLike, ArrayLike]:
...
def spherical_to_cartesian_vec(
	vx: ArrayLike, vy: ArrayLike, vz: ArrayLike, t: ArrayLike, p: ArrayLike
) -> tuple[ArrayLike, ArrayLike, ArrayLike]:
...
def clip_angle(angle: ArrayLike, max_value: float = 180) -> np.ndarray | float:
	...
	mod_angle = np.mod(angle_, 360.0)
	clipped_angle = np.where(mod_angle > max_value, mod_angle - 360.0, mod_angle)
...
def thompson_sphere(
	...
	t_r_min = (
		-(
			v_obs_x * (v_ref_x - v_obs_x)
			+ v_obs_y * (v_ref_y - v_obs_y)
			+ v_obs_z * (v_ref_z - v_obs_z)
		)
		/ r_obs_rs**2
	)
...
def los_rmin2angle(rmin_rs: ArrayLike, d_obs_rs: float) -> np.ndarray | float:
	...
	angle_deg = np.rad2deg(
		np.where(mask, np.pi - np.arcsin(2 - ratio, where=mask), np.arcsin(ratio, where=~mask))
	)
	...
	match coord_system.lower():
		case "lonlat" | "lonlatau":
			...
			columns = [
				coord.obstime,
				positions.distance,
				positions.lat.to(u.deg),
				positions.lon.to(u.deg),
			]
		case "rtp" | "spherical":
			names = ["time", "r", "t", "p"]
			...
			columns = [
				coord.obstime,
				positions.r.to(u.R_sun),
				positions.theta.to(u.rad),
				positions.phi.to(u.rad),
			]
		case "cartesian" | "xyz":
			names = ["time", "x", "y", "z"]
